...
import { GeneralContentOptions, IndexLinkStyle, ZoottelkeeperPluginSettings } from './interfaces'
...
export default class ZoottelkeeperPlugin extends Plugin {
    ...
    generateFormattedIndexItem = (path: string, isFile: boolean): string => {
        ...
        console.log("fileAbstrPath:", fileAbstrPath)
    ...
    generateIndexItem = (path: string, isFile: boolean): string => {
        ...
        if (this.settings.cleanPathBoolean) {
            ...
            if (isFile) {
                switch (this.settings.indexLinkStyle) {
                    case IndexLinkStyle.Title:
                        // Get title from frontmatter
                        const title: string = metadata.frontmatter['title'];
                        previewText = title && title;
                    case IndexLinkStyle.Alias:
                        // Get Alias from frontmatter
                        const alias = metadata.frontmatter['aliases'][0];
                        previewText = alias && alias;
                    case IndexLinkStyle.Filename:
                        previewText = fileName;
                    case IndexLinkStyle.Heading:
                        const heading = metadata.headings[0].heading;
                        previewText = heading && heading;
                };
            }

            internalFormattedIndex = `${cleanPath}|${previewText}`;
...
class ZoottelkeeperPluginSettingTab extends PluginSettingTab {
    ...
    display(): void {
        ...
        new Setting(containerEl)
            .setName('Link Style')
            .setDesc('Select the style of the file link preview text in index. ')
            .addDropdown(async (dropdown) => {
                dropdown.addOption(IndexLinkStyle.Filename, 'Name of the file');
                dropdown.addOption(IndexLinkStyle.Title, 'Title field from frontmatter');
                dropdown.addOption(IndexLinkStyle.Alias, 'First alias from frontmatter');
                dropdown.addOption(IndexLinkStyle.Heading, 'First heading in file');

                dropdown.setValue(this.plugin.settings.indexLinkStyle);
                dropdown.onChange(async (option) => {
                    console.debug('Chosen link preview style: ' + option);
                    this.plugin.settings.indexLinkStyle = option as IndexLinkStyle;
                    await this.plugin.saveSettings();
                });
            });
